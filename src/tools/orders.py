...
def load_orders_documents(file_path: str) -> multi_key_dict:
    ...
    Returns
        multi_key_dict: A multi-key dictionary of order documents, keyed by Order_ID, Order_Tracking, and Order_Email.
    """
...
@tool("search_order_catalog", description="Search for orders based on a key that is an order ID, tracking number, or email.")
def search_order_catalog(key: str, runtime: ToolRuntime):
    ...
    """
    return runtime.context[ORDERS_NAME].get(key, [])