...
def search_menu_catalog(query: str, runtime: ToolRuntime):
    """
    Search for menu items based on a query and return the top 3 most similar items from the vector store.

    Args:
        query (str): The search query.
        runtime (ToolRuntime): The runtime environment for the tool.
    """
    return runtime.context[MENU_COLLECTION_NAME].similarity_search(query, k=3)